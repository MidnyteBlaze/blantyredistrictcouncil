<?php namespace WHITEHALL\Includes\Classes;
/**
 * Common functions
 */
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => whitehall_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => whitehall_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => whitehall_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => whitehall_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => whitehall_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => whitehall_meta( 'banner_elementor_template', $page_id ),
				'banner'            => whitehall_set( whitehall_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => whitehall_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => whitehall_meta( 'banner_page_banner', $page_id ),
			];
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : whitehall_the_title( $template ),
				'banner'        => whitehall_set( $bg, 'url' ),
